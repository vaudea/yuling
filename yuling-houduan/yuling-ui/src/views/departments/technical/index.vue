...
export default {
  ...
  methods: {
    // 获取部门高级管理人员信息
    getPeople() {
      request.get("/department/technical/getPeople").then((res) => {
        if (res.code == 200) {
    ...
    getList() {
      this.loading = true;
      request.get("/department/technical/getList", { params: this.queryParams }).then((res) => {
        if (res.code == 200) {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.departmentId != null) {
            ...
            request.put("/department/technical/updateDepartment", this.form).then((res) => {
              if (res.code == 200) {
                this.open = false;
            ...
            request.post("/department/technical/insertDepartment", this.form).then((res) => {
              if (res.code == 200) {
                this.open = false;
    ...
    handleDelete(row) {
      this.$model.confirm(`确定要删除该部门信息吗?`).then(() => {
        return request.delete('/department/technical/deleteDepartment/' + row.departmentId);
      }).then((res) => {
        if (res.code == 200) {
